...
- [x] Fix any issues found before Step 10. — **One found and fixed, without a full usability pass:** the "smart-home aesthetic" visual revision (post-Step-9) collapsed the post-generate screen's two working links (visible payer URL + a separate deeplink anchor, working since Step 4) into a single `<a href="nimiqpay://...">` "Share link" button. Tapping it from *inside* Nimiq Pay's own WebView (self-referential custom-scheme navigation) silently did nothing — a real regression, caught by real device testing, not the usability pass. Fixed: the payer URL is visible/selectable text again, plus a "Copy link" button (Clipboard API) as the primary action, with the deeplink kept as a secondary option. **Needs on-device confirmation** that Copy link actually copies and that the copied URL opens `PayerView` correctly — not yet confirmed as of this entry.
