...
2. **New, real blocker surfaced by provisioning it: network mismatch.** `NIMIQ_NETWORK=testnet` in `.env.local`, but `rpc.nimiqwatch.com` is **mainnet**. If the funded test wallet is testnet (as `DECISIONS.md`'s "funded testnet wallet" phrasing from Step 5 suggests), a real broadcast from it will never be found by this RPC — indistinguishable, from the outside, from a genuine timeout. **Must be resolved (testnet RPC endpoint, or confirm the wallet is mainnet) before the real approve-path retest**, or that retest will misleadingly look like Step 6 is broken when it's actually a network mismatch.
3. **The full approve → broadcast → confirmed loop has still never run end to end against a real device**, per your note — deferred pending real NIM in the test wallet. What's now proven for real: the RPC leg of that loop (real endpoint, real 60s poll, correct timeout result). What's still unproven: the wallet-approval leg, and the "included and confirmed" happy path against a real transaction — a genuinely broadcast-but-pending transaction has never been checked, only a hash that was never broadcast at all (`DECISIONS.md` #6's "one nuance" note).
4. **Test data left on the real deployment** from this verification pass: one request (memo "RPC connectivity test") and its participant/event. No delete mutation exists (by design — no write path other than the intended ones), so this wasn't cleaned up automatically. Should be removed via the Convex dashboard before Step 11's demo rehearsals, so it doesn't show up in the dashboard list.
