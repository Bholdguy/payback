...
|---|---|---|
| A "payment-request method" returning a rich provider event | `sendBasicTransactionWithData({ recipient, value, data, fee?, validityStartHeight? })` — triggers the native approval dialog, resolves to a **transaction hash string** on approval, or does not resolve with one on rejection/failure (see the Step 5 real-device finding below — the specific `PermissionDeniedError`/`InvalidTransactionError` classes this doc originally cited from `nimiq.dev`'s API reference were never confirmed against the installed SDK or a real device) | Status read-back cannot rely on a rich "provider result object" (Section 4's `PaymentStatusReader`). It must be built from a tx hash + independent chain lookup. See below. |
...
| Provider event distinguishes "cancelled" vs "insufficient balance" | The API reference page names two error types (`PermissionDeniedError`, `InvalidTransactionError`), but neither exists in the installed SDK's source, and a real device only confirmed the rejection path (named "Reject" button, distinct from cancel/dismiss) produces a non-success outcome — insufficient-balance was never tested (see Section 5.3's real-device finding, `DECISIONS.md`) | The app cannot reliably tell "user tapped Reject" apart from "wallet rejected for balance reasons" from the SDK alone, and now doesn't try to via error class — both are treated identically: **not paid, offer retry**, triggered by "did not resolve with a tx hash," not by matching a specific error type. |
...
- `failed` — the SDK-level trigger is now stated behaviorally rather than by named error class: **`sendBasicTransactionWithData()` does not resolve with a successful transaction hash** (whether that means it rejects with a thrown value, or resolves with a non-string/error-shaped result — see the real-device finding below), or the confirmation job's timeout elapses with no inclusion found; retry is offered in either case.

  **Real-device finding (Step 5, confirmed on-device):** `nimiq.dev/mini-apps/api-reference/nimiq-provider` documents `PermissionDeniedError`/`InvalidTransactionError` as named, thrown error types for `sendBasicTransactionWithData()`. Neither class exists anywhere in the installed `@nimiq/mini-app-sdk@0.1.0` package source (its static types show the call *resolving* to `string | ErrorResponse`, never throwing a named class — see `src/nimiq/provider.ts`). A real on-device test (tapping "Reject" on the native "Confirm Transaction" dialog, which has explicit Confirm/Reject buttons, not a swipe-dismiss) confirmed the app-level behavior is correct — the rejected participant showed "Payment not completed. Try again." with a working retry, the other three participants were unaffected, and no false-paid status appeared — but the device had no remote-debugging access, so the *exact* rejected/resolved value's shape was never captured from a raw log, only inferred from correct on-screen behavior. The failed-trigger condition above is written to be true regardless of which shape it turns out to be, precisely because that exact shape is still unverified. Insufficient-balance was not tested at all (skipped to avoid risking the funded testnet wallet) and remains a fully untested edge case. See `DECISIONS.md` for the full disclosure.
